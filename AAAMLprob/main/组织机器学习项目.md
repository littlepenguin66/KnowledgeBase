...
![](https://cdn.jsdelivr.net/gh/littlepenguin66/webImage/AAAMLP_page74_image.png)

<p align="center"><b>图 1：CSV格式的 MNIST 数据集</b> </p>

让我们来看看这个数据集中标签列的计数。

![](https://cdn.jsdelivr.net/gh/littlepenguin66/webImage/AAAMLP_page74_image_1.png)
...
model_dispatcher.py 从 scikit-learn 中导入了 tree([tree.DecisionTreeClassifier](https://scikit-learn.org.cn/view/784.html))，并定义了一个字典，其中键是模型的名称，值是模型本身。在这里，我们定义了两种不同的决策树，一种使用基尼标准，另一种使用熵标准。要使用 py，我们需要对训练脚本做一些修改。
...
我使用了一些脚本，如 _model_dispatcher.py_ 和 _config.py_，并将它们导入到我的训练脚本中。请注意，我没有导入 _，你也不应该导入。如果我导入了 _，你就永远不会知道模型字典是从哪里来的。编写优秀、易懂的代码是一个人必须具备的基本素质，但许多数据科学家却忽视了这一点。如果你所做的项目能让其他人理解并使用，而无需咨询你的意见，那么你就节省了他们的时间和自己的时间，可以将这些时间投入到改进你的项目或开发新项目中去。
